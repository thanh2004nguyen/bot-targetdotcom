...
def click_place_order_button(sb):
    # Click "Place Order" button and enter CVV with improved reliability
    ...
    while retry_count < max_retries:
        try:
            ...
            # Method 1: Find by data-test="placeOrderButton" (most reliable)
            try:
                ...
                for button in place_order_buttons:
                    try:
                        if button.is_displayed() and button.is_enabled():
                            ...
                            print("Place Order button clicked (by data-test).")
                            
                            # Wait a moment for page to update
                            time.sleep(0.5)
                            break
                    except Exception as e:
                        print(f"Error clicking button in loop: {e}")
                        continue
            except Exception as e:
                print(f"Error finding buttons by data-test: {e}")
            
            # Method 2: Try SeleniumBase click if Method 1 failed
            if not place_order_clicked:
                try:
                    if is_element_visible_with_timeout(sb, 'button[data-test="placeOrderButton"]', timeout=3):
                        # Scroll into view first
                        try:
                            element = sb.find_element('button[data-test="placeOrderButton"]', timeout=2)
                        ...
                        place_order_clicked = True
                        print("Place Order button clicked (by SeleniumBase).")
                        time.sleep(0.5)
                except Exception as e:
                    print(f"Error with SeleniumBase click: {e}")
            
            # Method 3: Try finding by text "Place your order" (priority) or "Place Order"
            if not place_order_clicked:
                try:
                    all_buttons = sb.driver.find_elements(By.TAG_NAME, 'button')
                    for button in all_buttons:
                        try:
                            ...
                            # Priority: "Place your order" (case-insensitive)
                            if button.is_displayed() and button.is_enabled():
                                if "place your order" in button_text_lower or "place order" in button_text_lower:
                                    sb.driver.execute_script("arguments[0].scrollIntoView({block: 'center', behavior: 'smooth'});", button)
                                    time.sleep(0.3)
                                    button.click()
                                    place_order_clicked = True
                                    print(f"Place Order button clicked (by text: '{button_text}').")
                                    time.sleep(0.5)
                                    break
                        except:
                            continue
            ...
            if not place_order_clicked:
                ...
                if retry_count < max_retries:
                    print(f"Place Order button not found. Retrying ({retry_count}/{max_retries})...")
                    time.sleep(1)
                    continue
                else:
                    print("Place Order button not found after all retries.")
                    return False
            ...
            try:
                ...
                try:
                    ...
                    # If CVV field not found, retry clicking Place Order button
                    retry_count += 1
                    if retry_count < max_retries:
                        print(f"Retrying Place Order click ({retry_count}/{max_retries})...")
                        time.sleep(1)
                        continue
                    else:
                        return False
            ...
            print(f"Error in click_place_order_button (attempt {retry_count + 1}): {e}")
            retry_count += 1
            if retry_count < max_retries:
